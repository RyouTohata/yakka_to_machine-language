...
使い方:
    python mhlw_price_converter_v4.py --input-dir . --output-dir output

任意:
    python mhlw_price_converter_v4.py --input-dir . --output-dir output --manual-aliases manual_aliases.json
"""
...
def derive_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 後発側価格記号
    df["generic_price_marker"] = raw_j.where(raw_j.isin(["★"]), "")

    # 先発側価格記号
    df["originator_price_marker"] = raw_l.where(raw_l.isin(["☆"]), "")
...
def build_meta_info(source_files: list[str]) -> MetaInfo:
    return MetaInfo(
        ...
        generator="mhlw_price_converter_v4",
